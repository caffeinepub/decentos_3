import { Check, Copy, Download, Heart, Palette, Trash2 } from "lucide-react";
...
function generatePalette(base: string, harmony: HarmonyType): string[] {
...
export function ColorPaletteGenerator() {
  const [baseColor, setBaseColor] = useState("#27D7E0");
  const [harmony, setHarmony] = useState<HarmonyType>("complementary");
  const [palette, setPalette] = useState<string[]>([]);
  const [copiedIdx, setCopiedIdx] = useState<number | null>(null);
  const { data: saved, set: setSavedChain } = useCanisterKV<SavedPalette[]>(
    ...
    >
      ...
      >
        <div className="flex items-center gap-1">
          <Palette className="w-4 h-4" style={{ color: "var(--os-accent)" }} />
          <span
        ...
        <div>
          ...
          <div className="flex gap-2 h-32">
            {palette.map((color, i) => (
              ...
              >
                {copiedIdx === i ? (
                  <Check className="w-4 h-4 absolute top-2 right-2 text-foreground" />
                ) : (
                  <Copy className="w-3 h-3 absolute top-2 right-2 opacity-50 text-foreground" />
                )}
                <span
