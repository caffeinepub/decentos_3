...
function renderMarkdown(md: string): string {
  ...
  html = html.replace(
    /```([\s\S]*?)```/g,
    (_m, code) =>
      `<pre style="background:rgba(0,0,0,0.4);border:1px solid rgba(42,58,66,0.8);border-radius:6px;padding:12px;overflow-x:auto;font-size:11px;color:rgba(39,215,224,0.85);font-family:monospace;margin:8px 0">${code.trim()}</pre>`,
  );
  html = html.replace(
    /`([^`]+)`/g,
  ...
  html = html.replace(
    /(<li[^>]*>[\s\S]*?<\/li>\n?)+/g,
    (match) =>
      `<ul style="margin:8px 0;padding-left:20px;list-style:disc;color:rgba(180,200,210,0.75)">${match}</ul>`,
  );
  html = html.replace(
    /\n\n+/g,
    ...
    <div className="flex h-full" style={{ background: "rgba(11,15,18,0.6)" }}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div
          ...
          // biome-ignore lint/security/noDangerouslySetInnerHtml: markdown renderer output
