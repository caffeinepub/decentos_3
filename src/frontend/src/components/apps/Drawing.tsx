...
const TOOLS: ToolDef[] = [
  ...
  { id: "eraser", icon: <Minus className="w-4 h-4" />, label: "Eraser" },
...
const TEXT_FONT_SIZES: { label: string; size: number }[] = [
  { label: "S", size: 14 },
  { label: "M", size: 20 },
  { label: "L", size: 28 },
];
...
export function Drawing() {
  ...
  const { data: savedDrawing, set: setSavedDrawing } = useCanisterKV<string>(
    "decentos_drawing",
    "",
  );
  const savedDrawingRef = useRef(savedDrawing);
  savedDrawingRef.current = savedDrawing;
  const setSavedDrawingRef = useRef(setSavedDrawing);
  ...
  const colorRef = useRef("#27D7E0");
  const strokeWidthRef = useRef(3);
  const textSizeRef = useRef(20);

  const [tool, setToolState] = useState<Tool>("pen");
  const [color, setColorState] = useState("#27D7E0");
  const [strokeWidth, setStrokeWidthState] = useState(3);
  const [historyLen, setHistoryLen] = useState(0);
  const [showClearConfirm, setShowClearConfirm] = useState(false);
  const redoStack = useRef<ImageData[]>([]);
  const [textFontSizeLabel, setTextFontSizeLabel] = useState("M");
  ...
  // Load saved drawing once after canvas initializes
  ...
  useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      if (currentTool === "text") {
        const pos = getPos(e);
        setTextOverlay({ x: pos.x, y: pos.y, value: "" });
        return;
      }
    ...
    const onMouseUp = () => {
      ...
      // Save to chain on draw complete
        ...
        <div className="flex items-center gap-1">
          {TOOLS.map((t) => (
            <button
              ...
              style={{
                background:
                  tool === t.id ? "rgba(39,215,224,0.2)" : "transparent",
      ...
      {showClearConfirm && (
        ...
        >
          ...
          >
            ...
            >
              This will erase all content on the canvas. This action can be
              undone.
            </p>
