import { c as createLucideIcon, r as reactExports, h as useWindowFocus, j as jsxRuntimeExports, ad as Clock } from "./index-8tMpYjTW.js";
/**
...
function Calculator() {
  ...
  const [prevValue, setPrevValue] = reactExports.useState(null);
  const [pendingOp, setPendingOp] = reactExports.useState(null);
  const [waitingForOperand, setWaitingForOperand] = reactExports.useState(false);
  const [expression, setExpression] = reactExports.useState("");
  const [history, setHistory] = reactExports.useState([]);
  const [showHistory, setShowHistory] = reactExports.useState(false);
  const [pressed, setPressed] = reactExports.useState(null);
  const pressedRef = reactExports.useRef(setPressed);
  pressedRef.current = setPressed;
  const animatePress = reactExports.useCallback((key) => {
    pressedRef.current(key);
    setTimeout(() => pressedRef.current(null), 120);
  }, []);
  const inputDigit = reactExports.useCallback(
    (digit) => {
      animatePress(digit);
      if (waitingForOperand) {
        ...
        setDisplay(
          (prev) => prev === "0" ? digit : prev.length >= 15 ? prev : prev + digit
        );
      }
    },
    [waitingForOperand, animatePress]
  );
  const inputDot = reactExports.useCallback(() => {
    animatePress(".");
    if (waitingForOperand) {
      setDisplay("0.");
      setWaitingForOperand(false);
  ...
  const clearEntry = reactExports.useCallback(() => {
    animatePress("CE");
    setDisplay("0");
  }, [animatePress]);
  const toggleSign = reactExports.useCallback(() => {
    animatePress("+/-");
    setDisplay(
      (prev) => prev.startsWith("-") ? prev.slice(1) : prev === "0" ? "0" : `-${prev}`
    );
  }, [animatePress]);
  const percent = reactExports.useCallback(() => {
    animatePress("%");
  ...
  const handleOp = reactExports.useCallback(
    (op) => {
      animatePress(op ?? "");
  ...
  const calculate = reactExports.useCallback(() => {
    animatePress("=");
  ...
  const btnBase = "h-12 rounded-lg text-sm font-semibold transition-all duration-75 select-none flex items-center justify-center cursor-pointer active:scale-95";
  const btnDigit = "bg-white/5 border border-white/10 hover:bg-white/10 text-foreground";
  const btnOp = "bg-primary/15 border border-primary/30 hover:bg-primary/25 text-primary";
  const btnAction = "bg-white/8 border border-white/15 hover:bg-white/15 text-muted-foreground hover:text-foreground";
  const btnEquals = "bg-primary/80 hover:bg-primary border-0 text-black font-bold col-span-1";
  const buttons = [
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        showHistory && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto", children: history.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground/40 p-3", children: "No history yet." }) : history.map((h, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[9px] text-muted-foreground/50 font-mono", children: h.expression }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs font-semibold text-primary font-mono", children: h.result })
                  ]
                },
                `${i}-${h.result}`
              )) })
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex flex-col p-3 gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              style: {
                ...
                boxShadow: "0 0 20px rgba(39,215,224,0.06), inset 0 1px 0 rgba(39,215,224,0.08)"
              },
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] font-mono text-muted-foreground/50 h-4 truncate max-w-full", children: expression || " " }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    className: "font-mono font-bold text-foreground truncate max-w-full",
                    style: {
                      fontSize: display.length > 12 ? "18px" : display.length > 8 ? "22px" : "28px",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(Delete, { className: "w-3.5 h-3.5" }),
                  "Del"
                ]
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                style: {
                  background: showHistory ? "rgba(39,215,224,0.15)" : "var(--os-border-subtle)",
                  border: `1px solid ${showHistory ? "rgba(39,215,224,0.4)" : "var(--os-text-muted)"}`,
                  color: showHistory ? "rgba(39,215,224,1)" : "var(--os-text-secondary)"
                },
                children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-4 gap-2 flex-1", children: buttons.map((btn) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
              onClick: btn.action,
              "data-ocid": "calculator.button",
              className: `${btnBase} ${btn.type === "digit" ? btnDigit : btn.type === "op" ? btnOp : btn.type === "equals" ? btnEquals : btnAction}`,
              style: {
